...
#ifdef __riscv
            // Whole 32-state loop hand-written in scalar FP asm: pointer-walk (no
            // per-iteration array-index math), FP kept in registers, one branch.
            // Same math as the C fallback. -O0/-O2/-O3 all run this identical block.
            {
                const float *ap=A_bar_real_arr,*aip=A_bar_imag_arr,*bp=B_bar_real_arr,
                            *bip=B_bar_imag_arr,*cp=C_real_arr,*cip=C_imag_arr;
                float *xrp=x_real,*xip=x_imag; int cnt=half_state;
                __asm__(
                 "1:\n\t"
                 "flw fa0,0(%[ap])\n\t  flw fa1,0(%[aip])\n\t"
                 "flw fa2,0(%[xrp])\n\t flw fa3,0(%[xip])\n\t"
                 "fmul.s ft0,fa0,fa2\n\t  fnmsub.s ft0,fa1,fa3,ft0\n\t"   /* dr = ar*xr-ai*xi */
                 "fmul.s ft1,fa0,fa3\n\t  fmadd.s  ft1,fa1,fa2,ft1\n\t"   /* di = ar*xi+ai*xr */
                 "flw fa4,0(%[bp])\n\t  flw fa5,0(%[bip])\n\t"
                 "fmadd.s ft0,fa4,%[u],ft0\n\t  fmadd.s ft1,fa5,%[u],ft1\n\t" /* x = decayed+B*u */
                 "fsw ft0,0(%[xrp])\n\t  fsw ft1,0(%[xip])\n\t"
                 "flw fa6,0(%[cp])\n\t  flw fa7,0(%[cip])\n\t"
                 "fmul.s ft2,fa6,ft0\n\t  fnmsub.s ft2,fa7,ft1,ft2\n\t"   /* term = cr*xr-ci*xi */
                 "fmadd.s %[y],%[two],ft2,%[y]\n\t"                          /* y += 2*term */
                 "addi %[ap],%[ap],4\n\t  addi %[aip],%[aip],4\n\t"
                 "addi %[bp],%[bp],4\n\t  addi %[bip],%[bip],4\n\t"
                 "addi %[cp],%[cp],4\n\t  addi %[cip],%[cip],4\n\t"
                 "addi %[xrp],%[xrp],4\n\t addi %[xip],%[xip],4\n\t"
                 "addi %[cnt],%[cnt],-1\n\t  bnez %[cnt],1b\n\t"
                 : [y]"+f"(y),[ap]"+r"(ap),[aip]"+r"(aip),[bp]"+r"(bp),[bip]"+r"(bip),
                   [cp]"+r"(cp),[cip]"+r"(cip),[xrp]"+r"(xrp),[xip]"+r"(xip),[cnt]"+r"(cnt)
                 : [u]"f"(u_t),[two]"f"(two)
                 : "ft0","ft1","ft2","fa0","fa1","fa2","fa3","fa4","fa5","fa6","fa7","memory"
                );
            }
#else
            for (int n = 0; n < half_state; n++) {
