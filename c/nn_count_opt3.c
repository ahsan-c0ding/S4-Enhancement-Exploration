    ...
    for (int h = 0; h < D_MODEL; h++) {
        ...
        for (int t = 0; t < SEQ_LEN; t++) {
            ...
            for (int n = 0; n < half_state; n++) {
                ...
                // y_t += 2*Re(C * x_t). Only the REAL part is used, so compute it
                // directly instead of a full complex_mul (drops the dead imag half).
                float C_real_val = C_real[(h * half_state + n) * 2];
                float C_imag_val = C_imag[(h * half_state + n) * 2];
                y += 2.0f * (C_real_val * x_real[n] - C_imag_val * x_imag[n]);
            }
