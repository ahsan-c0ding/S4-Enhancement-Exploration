...
#ifdef __riscv
            // Vectorized recurrent update over all 32 states (same math, RVV).
            {
                float acc = 0.0f; size_t vl;
                for (int n0 = 0; n0 < half_state; n0 += (int)vl) {
                    vl = __riscv_vsetvl_e32m2(half_state - n0);
                    vfloat32m2_t vxr = __riscv_vle32_v_f32m2(&x_real[n0], vl);
                    vfloat32m2_t vxi = __riscv_vle32_v_f32m2(&x_imag[n0], vl);
                    vfloat32m2_t var = __riscv_vle32_v_f32m2(&A_bar_real_arr[n0], vl);
                    vfloat32m2_t vai = __riscv_vle32_v_f32m2(&A_bar_imag_arr[n0], vl);
                    vfloat32m2_t vbr = __riscv_vle32_v_f32m2(&B_bar_real_arr[n0], vl);
                    vfloat32m2_t vbi = __riscv_vle32_v_f32m2(&B_bar_imag_arr[n0], vl);
                    vfloat32m2_t vcr = __riscv_vle32_v_f32m2(&C_real_arr[n0], vl);
                    vfloat32m2_t vci = __riscv_vle32_v_f32m2(&C_imag_arr[n0], vl);
                    // decayed = A_bar * x   (complex)
                    vfloat32m2_t vdr = __riscv_vfmul_vv_f32m2(var, vxr, vl);
                    vdr = __riscv_vfnmsac_vv_f32m2(vdr, vai, vxi, vl);   // ar*xr - ai*xi
                    vfloat32m2_t vdi = __riscv_vfmul_vv_f32m2(var, vxi, vl);
                    vdi = __riscv_vfmacc_vv_f32m2(vdi, vai, vxr, vl);    // ar*xi + ai*xr
                    // x = decayed + B_bar * u_t   (u_t scalar)
                    vxr = __riscv_vfmacc_vf_f32m2(vdr, u_t, vbr, vl);
                    vxi = __riscv_vfmacc_vf_f32m2(vdi, u_t, vbi, vl);
                    __riscv_vse32_v_f32m2(&x_real[n0], vxr, vl);
                    __riscv_vse32_v_f32m2(&x_imag[n0], vxi, vl);
                    // term = Re(C * x) = cr*xr - ci*xi ; sum it
                    vfloat32m2_t vt = __riscv_vfmul_vv_f32m2(vcr, vxr, vl);
                    vt = __riscv_vfnmsac_vv_f32m2(vt, vci, vxi, vl);
                    vfloat32m1_t vz = __riscv_vfmv_v_f_f32m1(0.0f, __riscv_vsetvlmax_e32m1());
                    vfloat32m1_t vred = __riscv_vfredosum_vs_f32m2_f32m1(vt, vz, vl);
