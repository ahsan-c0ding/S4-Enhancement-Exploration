...
void s4d_layer(
    ...
    uint64_t _disc=0,_scan=0,_a,_b;
    ...
    for (int h = 0; h < D_MODEL; h++) {
        ...
        _b=rdinstret(); _disc+=_b-_a; _a=_b;
        float x_real[32] = {0};
        float x_imag[32] = {0};
        ...
        for (int t = 0; t < SEQ_LEN; t++) {
            ...
            for (int n = 0; n < half_state; n++) {
#ifdef __riscv
                // Hand-written scalar FP asm for the whole complex state update +
                // read-out. Keeps everything in FP registers and removes the
                // complex_mul function call (a real jal/ret at -O0). Mirrors the C
                // fallback below exactly.
                float ar=A_bar_real_arr[n], ai=A_bar_imag_arr[n];
                float br=B_bar_real_arr[n], bi=B_bar_imag_arr[n];
                float cr=C_real_arr[n], ci=C_imag_arr[n];
                float xr=x_real[n], xi=x_imag[n];
                float nxr, nxi, tr;
                __asm__(
                    "fmul.s   %[nxr], %[ar], %[xr]\n\t"          /* nxr = ar*xr            */
                    "fnmsub.s %[nxr], %[ai], %[xi], %[nxr]\n\t"  /* nxr = ar*xr - ai*xi=dr */
                    "fmul.s   %[nxi], %[ar], %[xi]\n\t"          /* nxi = ar*xi            */
                    "fmadd.s  %[nxi], %[ai], %[xr], %[nxi]\n\t"  /* nxi = ar*xi + ai*xr=di */
                    "fmadd.s  %[nxr], %[br], %[u], %[nxr]\n\t"   /* nxr = dr + br*u        */
                    "fmadd.s  %[nxi], %[bi], %[u], %[nxi]\n\t"   /* nxi = di + bi*u        */
                    "fmul.s   %[tr],  %[cr], %[nxr]\n\t"         /* tr  = cr*nxr           */
                    "fnmsub.s %[tr],  %[ci], %[nxi], %[tr]\n\t"  /* tr  = cr*nxr - ci*nxi  */
                    "fmadd.s  %[y],   %[two], %[tr], %[y]\n\t"   /* y  += 2*tr             */
                    : [nxr]"=&f"(nxr), [nxi]"=&f"(nxi), [tr]"=&f"(tr), [y]"+f"(y)
                    : [ar]"f"(ar),[ai]"f"(ai),[br]"f"(br),[bi]"f"(bi),
                      [cr]"f"(cr),[ci]"f"(ci),[xr]"f"(xr),[xi]"f"(xi),
                      [u]"f"(u_t),[two]"f"(two)
                );
                x_real[n]=nxr; x_imag[n]=nxi;
#else
    ...
    printf("S4DSPLIT discretize=%llu scan=%llu\n",(unsigned long long)_disc,(unsigned long long)_scan);
}
