...
def evaluate_layer(name, mse, mae):
    ...
    if name == "TakeLastTimestep" and status == "FAIL": status = "FAIL*"
    return {"mse": mse, "mae": mae, "t_mse": target_mse, "t_mae": target_mae, "status": status}

print("\n[START] INITIATING AUTOMATED TASK 2 VALIDATION SUITE")
start_time = time.time()
master_results = []
e2e_agreements = 0

# Universal QEMU Locator (Handles TA environment and your environment)
qemu_path = shutil.which("qemu-riscv32")
if not qemu_path:
    for p in ["./qemu/bin/qemu-riscv32", "/opt/riscv/bin/qemu-riscv32", "/opt/riscv32imfcv/bin/qemu-riscv32"]:
        if os.path.exists(p):
            qemu_path = p
            break
if not qemu_path: qemu_path = "qemu-riscv32"

# Universal GCC Locator
gcc_path = shutil.which("riscv32-unknown-elf-gcc")
if not gcc_path:
    for p in ["/opt/riscv/bin/riscv32-unknown-elf-gcc", "/opt/riscv32imfcv/bin/riscv32-unknown-elf-gcc"]:
        if os.path.exists(p):
            gcc_path = p
            break
if not gcc_path: gcc_path = "riscv32-unknown-elf-gcc"
...
for i in range(NUM_SAMPLES):
    ...
    with open("qemu_compile.s", "w") as outfile:
        for fname in ["main_qemu.s", "nn_qemu.s", "math_qemu.s", f"data_{i}.s"]:
    ...
    compile_cmd = [
        gcc_path, "-march=rv32gcv", "-mabi=ilp32f", 
    ...
    try:
        ...
        print(f"[ERROR] QEMU Timed Out for Sample {i}.")
        continue
...
print("\n" + "="*70)
print(f"[DONE] ALL SIMULATIONS COMPLETED IN {(time.time()-start_time)/60:.1f} MINS.")
print(f"[SUCCESS] End-to-End Agreement: {e2e_agreements}/{NUM_SAMPLES} ({(e2e_agreements/NUM_SAMPLES)*100}%)")
print("="*70 + "\n")
...
\vspace{1ex}
{\footnotesize \textit{*Note: TakeLastTimestep inherits upstream S4D error since it is tested end-to-end, exceeding the isolated target.}}
\end{table}
"""
print(latex_table_2)